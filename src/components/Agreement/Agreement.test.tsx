...
it('sends the form data on give consent clicked', () => {
  ...
  fireEvent.input(getByRole('textbox', { name: /name/i }), { target: { value: testData.name }})
  fireEvent.input(getByRole('textbox', { name: /email address/i }), { target: { value: testData.email }})
  fireEvent.click(getByRole('checkbox', { name: /receive newsletter/i }))
  fireEvent.click(getByRole('button', { name: /give consent/i }))

  expect(fetchApiMock).toHaveBeenCalledTimes(1);
  expect(fetchApiMock).toHaveBeenCalledWith('/consents', expect.objectContaining({
    method: 'POST',
...
it('shows consent giving error message', async () => {
  ...
  fireEvent.input(getByRole('textbox', { name: /name/i }), { target: { value: 'test name' }})
  fireEvent.input(getByRole('textbox', { name: /email address/i }), { target: { value: 'test@email.com' }})
  fireEvent.click(getByRole('checkbox', { name: /receive newsletter/i }))
  fireEvent.click(getByRole('button', { name: /give consent/i }))
